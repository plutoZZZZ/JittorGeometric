'''
Description: GAT graph convolutional operator with multi-head attention support
Author: lusz
...
class GATConv(MessagePassingNts):
    ...
    def __init__(self, in_channels: int, out_channels: int, e_num: int,
        ...
        # Attention vector for each head: (2 * out_channels, heads)
        # We'll flatten this to: (heads * 2 * out_channels,) for 2D operations
    ...
    def edge_forward(self, x, csc) -> Var:
        ...
        # 1. Compute attention scores with attention vector FIRST (GAT key difference)
        # self.att shape: [heads, 2 * out_channels]
        # e: [num_edges, heads]
        # Replace einsum with basic operations to avoid cupy dependency
        e = (x_3d * self.att.unsqueeze(0)).sum(dim=2)
        
        # 2. THEN apply non-linearity
        activated = jt.nn.leaky_relu(e, scale=self.negative_slope)
        
        # Edge softmax
        a = EdgeSoftmax(activated, csc)  # [num_edges, heads]
        
        # Apply dropout to attention
        if self.dropout > 0:
            a = jt.nn.dropout(a, p=self.dropout)
        
        # Extract source features: [num_edges, heads, out_channels]
