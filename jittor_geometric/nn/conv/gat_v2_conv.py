'''
Description: GATv2: Dynamic Graph Attention
Author: Assistant
Date: 2026-02-07
'''
...
class GATv2Conv(MessagePassingNts):
    r"""The graph convolutional operator from the `"How Attentive are Graph Attention Networks?" <https://arxiv.org/abs/2105.14491>`_ paper (GATv2).

    This is an improved version of GAT where the attention mechanism is dynamic
    and can attend to any node in the neighborhood. The key difference from GAT
    is that GATv2 applies the non-linearity before computing attention scores,
    making the attention function dynamic across nodes.

    Key difference from GAT:
    - GAT: Linear projection → Activation → Attention computation
    - GATv2: Linear projection → Concatenate src/dst → Activation → Attention computation
    """
    ...
    def edge_forward(self, x, csc) -> Var:
        # GATv2: Apply non-linearity before computing attention scores
        # This makes the attention dynamic across nodes
        x_activated = jt.nn.leaky_relu(x, scale=0.2)
        out = x_activated @ self.edge_weight
    ...
    def vertex_forward(self, x: Var) -> Var:
        # GATv2: No activation here, activation is applied in edge_forward
        out = x @ self.weight
        return out
