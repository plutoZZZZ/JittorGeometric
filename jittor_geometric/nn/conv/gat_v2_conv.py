'''
Description: GAT_V2 Convolution
Author: 
Date: 2025-02-26
'''
...
class GATConvV2(MessagePassingNts):
    r"""The graph attention operator from the `"How Attentive are Graph Attention Networks?" 
    <https://arxiv.org/abs/2105.14491>`_ paper.
    
    GAT_V2 fixes the static attention problem of GAT by applying the nonlinearity 
    before the attention weight multiplication, allowing for dynamic attention.
    
    The key difference from GAT:
    - GAT: e_ij = LeakyReLU(a · [Wh_i || Wh_j])
    - GAT_V2: e_ij = a · LeakyReLU(Wh_i || Wh_j)
    """
    ...
    def __init__(self, in_channels: int, out_channels: int, e_num: int,
        ...
        super(GATConvV2, self).__init__(**kwargs)
